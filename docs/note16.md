---
title: WebSocket前后端连接
date: 2024-4-21
tags:
- WebSocket
---

传统的Http链接，需要前端向后端发送一个请求，后端返回一个结果以后就会断开链接。而当我们需要实现实时通信，比如聊天功能的时候，总不能我聊一句。他聊一句就断开连接吧？所以需要全双工的WebSocket链接，通信一次以后不会断开链接。而且ws还能实现监听的功能（我虽然没向你发送请求，但是你得返回信息给我）
    ...
    public void sendMessage(String message){
        synchronized (this.session){        //不能改成final
            try{
                ...
                //如何是一个对象则可以尝试把message变成JSON字符串发送,比如导入FastJson的maven包
            }catch (IOException e){
                e.printStackTrace();
            }
        }
